      ...
      try {
        ...
        const [{ data: rows, error }, { data: trackedRows }, { data: snapRows }] = await Promise.all([
          ...
          supabase
            ...
            .eq("player_tag", tag)
            .limit(1),
          // Progression history for the trophy curve. Recorded for EVERY
          // tracked player at SNAPSHOT_EVERY_HOURS (20), not just boosted ones
          // — boost only moves the per-brawler detail from weekly to daily.
          // 400 rows is over a year at one a day, and retention ages them out
          // at 365, so the limit can never bite.
          supabase
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1280, margin: "0 auto", padding: "34px 5vw 60px" }}>
        ...
        {/* Trophy history, directly under the header card's own
            🏆 {live.trophies} and ABOVE PlayerInsights. Do not "tidy" it into
            those panels: they are explicitly "competitive Ranked only" and a
            trophy chart invites exactly the ladder-vs-ranked comparison they
            avoid, which is why it was pulled out of them on 2026-09-19. The
            header has always printed a live trophy count, so this is where a
            trophy stat already belongs.

            It renders on BOTH this page and the "Get better" chapter of
            /profile, deliberately. They are different audiences: /profile is
            account-gated and shows you your own curve, while this page is
            public and works for all 1,419 tracked players with snapshots —
            which is most of the reach. Returns null under 2 points, so an
            untracked or brand-new tag shows nothing rather than an empty
            frame. */}
